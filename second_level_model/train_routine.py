...
import torchcontrib
from second_level_model import helper,dataset,config
import transformers
import time 
import gc
from first_level_model import helper as h
from sklearn.model_selection import StratifiedKFold
def ce_loss(pred, truth, smoothing=False,trg_pad_idx=-1, eps=0.1):
    """Make the label smooth as explained here https://arxiv.org/pdf/1701.06548.pdf"""
...
def len_sampler_fun(train_dataset, batch_size):
    len_sampler = helper.LenMatchBatchSampler(
...
def fit(model,
    ...
    len_sampler = len_sampler_fun(train_dataset,batch_size)
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            for data in val_loader:
                ...
                val_jac += helper.jaccard_from_logits_string(
        ...
        if val_jac >= best_jac and cp:
            helper.save_model_weights(model, 'checkpoint.pt', verbose=0)
            best_jac = val_jac

        dt = time.time() - start_time
        lr = scheduler.get_lr()[0]
        print(f'Epoch {epoch + 1}/{epochs} total_step {total_steps} \t lr={lr:.1e} \t t={dt:.0f}s \t',
              end='')
...
def k_fold(df_train, df_test,
    ...
    time = 'model_'
    score = 0
    splits = list(StratifiedKFold(n_splits=k,shuffle=True, random_state=fold_seed).split(
    ...
    for i, (train_idx, val_idx) in enumerate(splits):
        print(f"\n-------------   Fold {i + 1}  -------------")
        h.seed_everything(model_seed)

        model = model.TweetCharModel(
            len_voc,
        ...
        print('\n- Training all layers: ')
        helper.unfreeze(model)
        n_parameters = helper.count_parameters(model)
        ...
        if cp:
            helper.load_model_weights(model, "checkpoint.pt", verbose=0)
        if save:
            helper.save_model_weights(
                model,
...
def infer(df_test,X_test, preds,len_voc, k=5):
    time = 'model_'
    model_seed = 2021
    ...
    wmodel = []
    for m in range(5):
        filename = f'{config.selected_model}_{time}_{m + 1}.pt'
        wmodel = helper.load_model_weights(model, filename, verbose=1, cp_folder=config.CP_PATH)
        pred_test = predict(
        wmodel, test_dataset, batch_size=config.VALID_BATCH_SIZE)
        pred_tests.append(pred_test)
